...
export default function Navbar() {
  ...
  const { isLoggedIn, user, logout, isAdmin } = useAuth(); // <-- use 'user'
  ...
  // Corrected useEffect to fetch avatar using user.$id
  useEffect(() => {
    const loadAvatar = async () => {
      try {
        if (user?.id) {
          const response = await fetch(
            `/api/getStudentProfile?userId=${user.id}`
          );
          const profile = await response.json();

          if (profile?.imageUrl) {
            setAvatarUrl(profile.imageUrl); // use the direct URL from DB
          } else {
