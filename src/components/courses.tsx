...
// // //           $id: doc.$id,
// // //           title: (doc as CourseDoc).title || "Untitled",
// // //           description: (doc as CourseDoc).description || "No description",
// // //           Price: (doc as CourseDoc).Price,
// // //           ImageUrl: (doc as CourseDoc).ImageUrl,
// // //           category: (doc as CourseDoc).category || "General",
// // //           syllabus: (doc as CourseDoc).syllabus || "No syllabus available",
// // //         }));
...
// // Interface for enrollment documents
// interface EnrollmentDoc extends Models.Document {
//   courseId?: string;
// }

// interface Course extends Partial<Models.Document> {
//   $id: string;
//   title: string;
//   description: string;
//   Price?: number;
...
//         const enrolled = new Set(
//           res.documents
//             .map((doc) => (doc as EnrollmentDoc).courseId)
...
//                   variant="contained"
//                   color="primary"
//                   onClick={() => selectedCourse && addToCart(selectedCourse)}
//                 >
//                   Add to Cart
//                 </Button>
//               )
...
// interface Course extends Partial<Models.Document> {
//   $id: string;
//   $sequence: number; // required number to fix TS error
//   title: string;
//     $collectionId: string;
//   description: string;
//   Price?: number;
...
//         const mappedCourses: Course[] = response.documents.map(
//           (doc: Models.Document) => {
//             const courseData = doc as unknown as Course; // cast to your Course type
//             return {
//               ...doc,
...
// // Unified Course interface with all required Appwrite fields
...
//                       addToCart({
//                         ...selectedCourse,
//                         $databaseId: selectedCourse.$databaseId ?? "",
//                         $collectionId: selectedCourse.$collectionId ?? "",
//                       });
//                   }}
//                 >
//                   Add to Cart
...
// Strictly typed Course interface
interface Course extends Models.Document {
  title: string;
  description: string;
  Price?: number;
...
// Strictly typed Enrollment interface
interface EnrollmentDoc extends Models.Document {
  studentId: string;
  courseId: string;
}
...
const CoursesComponent: React.FC = () => {
  ...
  useEffect(() => {
    const fetchUserEnrollments = async () => {
      if (!isLoggedIn || !user) return;
      try {
        const res = await databases.listDocuments<EnrollmentDoc>(
          process.env.NEXT_PUBLIC_APPWRITE_DATABASE_ID!,
          process.env.NEXT_PUBLIC_APPWRITE_ENROLLMENTS_COLLECTION_ID!
        );

        const enrolled = new Set(res.documents.map((doc) => doc.courseId));
        setEnrolledCourses(enrolled);
      } catch (err) {
        console.error("No enrollments found", err);
      }
  ...
  useEffect(() => {
    const fetchCourses = async () => {
      try {
        ...
        const mappedCourses: Course[] = response.documents.map((doc) => ({
          ...doc,
          $databaseId: doc.$databaseId ?? "",
          $collectionId: doc.$collectionId ?? "",
          $permissions: doc.$permissions ?? [],
